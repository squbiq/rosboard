 ...
 * @property {function(string): void} onConnectionState - called when the peer connection state changes.
 * @property {string} localCandidateHost - replaces mDNS host candidates with this address.
 */
...
class MediaMTXWebRTCReader {
  ...
  #onLocalCandidate(evt) {
    ...
    if (evt.candidate !== null) {
      const candidate = this.#normalizeLocalCandidate(evt.candidate);
      if (this.#sessionUrl === null) {
        this.#queuedCandidates.push(candidate);
      } else {
        this.#sendLocalCandidates([candidate]);
      }
    }
  }

  #normalizeLocalCandidate(candidate) {
    if (
      this.#conf.localCandidateHost === undefined ||
      !/\.local(?=\s)/.test(candidate.candidate)
    ) {
      return candidate;
    }
  ...
  #onConnectionState() {
    ...
    if (this.#conf.onConnectionState !== undefined) {
      this.#conf.onConnectionState(this.#pc.connectionState);
    }
